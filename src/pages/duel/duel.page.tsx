...
import { Network, STAGE_NORMAL_END, STAGE_UNEXPECTED_END, Te_GameStage } from "@components/network/Network";
...
import { DuelResultUnexpectedView, DuelResultView } from "@components/views/duel_result.view";
...
function DuelManager() {
	...
	const [duel_data] = createResource(async () => {
		try {
			const res = await api.get("/duel/session",{params:{duel_session_id:params.duel_session_id}});
			...
			after={(stage:Te_GameStage)=> {
				switch (stage) {
					case STAGE_NORMAL_END:
						return <DuelResultView duel_session_id={params.duel_session_id}/>;
				...
				<CursorMetadataProvider>
					<section class="flex flex-col overflow-hidden">
						<div class="flex w-full flex-grow max-h-100 overflow-hidden">
							<Control editor_api={editor_api} side={side_from_number(duel_data()?.side ?? 0)!} />
