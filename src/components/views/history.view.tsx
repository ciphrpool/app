...
import { batch, createComputed, createResource, createSignal, For, Match, onCleanup, onMount, Show, Switch } from 'solid-js';
...
function HistoryView(props: HistoryViewProps) {
    ...
    const [page, {refetch}] = createResource(async () => {
		try {
			const res = await api.get("/duel/history",{params:{
	...
	return (
		<section class="grow border-4 border-night-400 p-4 overflow-hidden select-none">
			...
			<div class="block select-none text-xl tracking-widest text-night-100">
				{">>"} Duel History {props.against ? 'against ' + props.against.username : ''}
			</div>
						...
						<For each={duels()}>
							{(duel) => (
								<tr class="hover:bg-night-600 hover:text-moon transition-bg ease-in-out"
									onclick={() => {
										navigate(`/duel/history/${duel.sessionId}`)
									}}
								>
									<Show when={props.full}>
										<td class="px-8 py-4 hover:text-pl1-200 transition-colors ease-in-out">
											<Switch>
												<Match when={duel.isP1}>
													<span title={"tag = #"+duel.p2Tag+", elo :" + duel.p2Elo}
														class='select-none'
													>
														{duel.p2Username}
													</span>
												...
												<Match when={!duel.isP1}>
													<span title={"tag = #"+duel.p1Tag+", elo :" + duel.p1Elo}
														class='select-none'
													>
														{duel.p1Username}
													</span>
									...
									<td class="px-8 py-4 hover:text-pl1-200 transition-colors ease-in-out">
										<Switch>
											<Match when={duel.isP1 && duel.duelOutcome === 'P1_WON'}>
												W
											</Match>
											<Match when={duel.isP1 && duel.duelOutcome === 'P2_WON'}>
												L
											</Match>
											<Match when={!duel.isP1 && duel.duelOutcome === 'P1_WON'}>
												L
											...
											<Match when={duel.duelOutcome === 'draw'}>
												=
											</Match>
										</Switch>
									</td>
