...
import { createResource, JSXElement, Match, Show, Suspense, Switch } from "solid-js";
...
function PlayerStatistic(props:PlayerStatisticProps) {
    return (
        <div class="flex flex-col gap-4 select-none">
            <span class="text-xl tracking-widest ">&gt&gt {props.username} statistics</span>
            <ul class="flex flex-col gap-4">
                <li><div class="flex flex-row gap-4 items-center"><EnergyIcon /> energy : {props.Energy}</div></li>
                <li><div class="flex flex-row gap-4 items-center"><EgoIcon forced_color={props.player} /> nb of claimed ego : {props.EgoCount}</div></li>
                <li><div class="flex flex-row gap-4 items-center"><CorruptedDataIcon /> Corrupted data : {props.CorruptedData}</div></li>
                <li><div class="flex flex-row gap-4 items-center"><EmotionalDataIcon /> Emotional data : {props.EmotionalData}</div></li>
                <li><div class="flex flex-row gap-4 items-center"><QuantumIcon /> Quantum data : {props.QuantumData}</div></li>
                <li><div class="flex flex-row gap-4 items-center"><LogicalDataIcon /> Logical data : {props.LogicalData}</div></li>
            </ul>
        </div>
    );
}
    ...
    return (
        <div class="flex flex-col gap-4">
            <Show when={props.result.winningMethod !== "draw"}>
                <div class="text-center "><span class="text-xl dm-mono-medium tracking-widest">{props.winner}</span> won by : {format_winning_method(props.result.winningMethod)}</div>
            </Show>
            <div class="text-center capitalize">{props.result.duelType} duel lasted for: {format_time_min(props.result.duration)}</div>
            <ResultEloPlayers 
                        ...
                        onclick={async () => {
                            try {
                                await api.post(
                                    ...
                                    {
                                        opponent_tag:(props.result.isP1) ? props.result.p2Tag : props.result.p1Tag,
                                    }
                                );
                            } catch (error) {
                                fault.major({
...
function ResultHeader(props:ResultHeaderProps) {
    return (
        <div class="flex flex-col text-xl dm-mono-medium tracking-widest">
            <div class={`flex gap-8 items-center p-4 ${props.bg_color} border-4 ${props.border_color}`}>
                <Avatar class="w-16" />
                <h1 title={`tag : #${props.tag}`} class="text-2xl">{props.username} won!</h1>
            </div>
        </div>
    );
}
...
function ResultElo(props:ResultEloProps) {
    ...
    return (
        <div class="grow dm-sans-800 tracking-widest text-center">{props.elo - props.elo_change} {elo_change}</div>
    );
}
...
export function DuelResultView(props:DuelResultViewProps) {
    ...
    const [result] = createResource(async () => {
        while (true) {
            try {
                const res = await api.get("/duel/result",{params:{
                ...
                ((error as AxiosError).response?.data as {error:string}|undefined)?.error === "invalid duel session") {
                    await new Promise(r => setTimeout(r, 1000)); // Sleep 1 seconds
                    continue;
                }
                fault.major({message:"Failed to retrieve the result the duel"})
                return null
            }
        }
    });
    ...
    return (
        <section class="w-full h-full p-4 flex flex-col items-center">
            <Suspense fallback={"loading"}>
                <Switch>
                    <Match when={result()?.isP1 && result()?.duelOutcome === "P1_WON"}>
                        <Frame>
                            <ResultHeader
                                bg_color="bg-pl1-600"
                                ...
                                profile_picture_url={result()?.p1ProfilePictureUrl!}
                            />
                            <ResultStatistic winner={result()?.p1Username!} result={result()!}/>
                        </Frame>
                    </Match>
                    <Match when={result()?.isP1 && result()?.duelOutcome === "P2_WON"}>
                        <Frame>
                            <ResultHeader
                                bg_color="bg-pl2-600"
                                ...
                                profile_picture_url={result()?.p2ProfilePictureUrl!}
                            />
                            <ResultStatistic winner={result()?.p2Username!} result={result()!}/>
                        </Frame>
                    </Match>
                    <Match when={!result()?.isP1 && result()?.duelOutcome === "P1_WON"}>
                        <Frame>
                    ...
                    <Match when={result()?.duelOutcome === "draw"}>
                        <Frame>
                            <div class="bg-ego-dark p-4 border-4 border-ego
