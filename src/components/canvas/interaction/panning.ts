...
export class PanningHandler {
    ...
    focus(position:Point,zoom:number) {
        ...
        if (zoom > this.zone_size) {
            let next_zone_idx_x = this.zone_idx_x * this.zone_size + Math.floor((position.x / this.grid.grid_width) * 2);
            let next_zone_idx_y = this.zone_idx_y * this.zone_size +  Math.floor((position.y / this.grid.grid_width) * 2);
            this.zone_size = zoom;
	...
	subscribe(src: Container) {
	    ...
	    src.onpointerup = (e) => {
            ...
            const next_x_idx = Math.max(Math.min((this.zone_idx_x + direction_x),this.zone_size-1),0);
            const next_y_idx = Math.max(Math.min((this.zone_idx_y + direction_y),this.zone_size-1),0);
	...
	hold(to:Point) {
        ...
        const max_distance = this.grid.grid_width * PanningHandler.PANNING_RADIUS_RATE;

        this.container.x = this.starting_point.x + Math.cos(angle) * Math.min(distance,max_distance);
        this.container.y = this.starting_point.y + Math.sin(angle) * Math.min(distance,max_distance);
    }
