...
export function GameState(props:GameStateProps) {
    ...
    <div class="flex flex-col h-full">
        ...
        <div class="flex-grow flex items-center">
            <div class="grid grid-cols-[24px_1fr] gap-x-4 gap-y-4">
                <CorruptedDataIcon />
                <p>{get_corruption_data().current} | {get_corruption_data().max}</p>
                <EmotionalDataIcon />
                <p>{get_emotional_data().current} | {get_emotional_data().max}</p>
                <LogicalDataIcon />
                <p>{get_logical_data().current} | {get_logical_data().max}</p>
                <QuantumIcon />
                <p>{get_quantum_data().current} | {get_quantum_data().max}</p>
            </div>
        </div>
    </div>
    ...
    <div class="col-span-2 flex flex-col items-center gap-4 h-full">
        ...
        <span class="flex flex-row gap-4 items-center flex-grow-0">
            ...
            <p class="text-pl1-200">{get_ego_count().p1}</p> | <p class="text-pl2-200">{get_ego_count().p2}</p>
        </span>
    </div>
