...
const EDITOR_READONLY_LISTENERS : Map<Te_Cursor,{
    on_key_down: IDisposable,
    on_did_change_cursor_selection: IDisposable,
    on_did_change_model_content: IDisposable,
}> = new Map();

export function to_readonly(cursor:Te_Cursor,until:number,editor: monaco_editor.IStandaloneCodeEditor) {
    ...
    const on_did_change_cursor_selection = editor.onDidChangeCursorSelection(_ => {
    ...
    EDITOR_READONLY_LISTENERS.set(cursor,{on_key_down,on_did_change_cursor_selection,on_did_change_model_content});
}


export function undo_readonly(cursor:Te_Cursor,editor: monaco_editor.IStandaloneCodeEditor) {
    const model = editor.getModel();
    if (!model) return;
    ...
    const readOnlyDecorations = allDecorations?.filter(decoration => decoration.options.className === 'editor-line-readonly');
