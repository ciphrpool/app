import { For, JSXElement, createContext, createSignal, onMount, useContext } from "solid-js";
import { Portal } from "solid-js/web"

type fault_type = "critical_fault" | "major_fault" | "minor_fault" | "info_fault"
const INFO = Symbol("INFO")
const MINOR = Symbol("MINOR")
const MAJOR = Symbol("MAJOR")
const CRITICAL = Symbol("CRITICAL")
export type Te_fault_severity = typeof INFO | typeof MINOR | typeof MAJOR | typeof CRITICAL;
...
class FaultEvent extends Event {
    ...
    private constructor(type:fault_type,severity:Te_fault_severity,detail: ErrorMsg) {
      super(type);
      this.detail = detail;
    ...
    static MINOR(detail?:ErrorMsg) {
        return new FaultEvent("minor_fault",MINOR,detail ?? {message:null})
    }
    static MAJOR(detail?:ErrorMsg) {
        return new FaultEvent("major_fault",MAJOR,detail ?? {message:null})
    }
    static CRITICAL(detail?:ErrorMsg) {
        return new FaultEvent("critical_fault",CRITICAL,detail ?? {message:null})
    }
}
...
function FaultPopup(props:FaultProps) {
    ...
    onMount(() => {
        if ( props.severity === INFO ||props.severity === MINOR ||props.severity === MAJOR){
            setTimeout(() => {
    ...
    return <div ref={ref} class="flex gap-4 justify-between h-fit border-2 border-moon text-moon font-normal text-wrap transition-opacity duration-500 show"
        classList={{
    ...
    >
        ...
        <p class="p-4 cursor-pointer text-moon font-semibold" onclick={() => {
            ref.classList.replace("show","hide");
            ref.onanimationend = () => {
                props.remove()
            }
...
export function FaultHandler(props:FaultHandlerProps) {
    const [get_faults,set_faults] = createSignal<{event:FaultEvent,id:number}[]>();
    ...
    fault.addEventListener("critical_fault",(event) => {
        set_faults([...get_faults() ?? [],{event:(event as FaultEvent),id:auto_increment++}])
    });
            ...
            <For each={get_faults()}>
                {({event:fault,id:own_id}) => 
                    <FaultPopup 
                        ...
                        remove={() => {
                            const faults = get_faults() ?? [];
                            set_faults(faults.filter(({id}) => id !== own_id));
                        }}
                    >
